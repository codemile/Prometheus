namespace Prometheus.Grammar
{
    ...
    public enum GrammarRule
    {
        ...
        @FlowControl8 = 165, // <FlowControl> ::= <EachControl>
        @FlowControl9 = 166, // <FlowControl> ::= <BreakControl>
        @FlowControl10 = 167, // <FlowControl> ::= <ContinueControl>
        @IfControl_if_LParen_RParen = 168, // <IfControl> ::= if '(' <Expression> ')' <Block>
        @IfControl_if_LParen_RParen_else = 169, // <IfControl> ::= if '(' <Expression> ')' <Block> else <Block>
        @DoWhileControl_while_LParen_RParen = 170, // <DoWhileControl> ::= while '(' <Expression> ')' <Block>
        @DoUntilControl_until_LParen_RParen = 171, // <DoUntilControl> ::= until '(' <Expression> ')' <Block>
        @LoopWhileControl_do_while_LParen_RParen = 172, // <LoopWhileControl> ::= do <Block> while '(' <Expression> ')'
        @LoopUntilControl_do_until_LParen_RParen = 173, // <LoopUntilControl> ::= do <Block> until '(' <Expression> ')'
        @ForControl_for_Identifier_Eq_to = 174,
        // <ForControl> ::= for Identifier '=' <Expression> to <Expression> <Block>
        @ForStepControl_for_Identifier_Eq_to_step = 175,
        // <ForStepControl> ::= for Identifier '=' <Expression> to <Expression> step <Expression> <Block>
        @EachControl_each = 176, // <EachControl> ::= each <Qualified ID> <Block>
        @EachControl_each_as = 177, // <EachControl> ::= each <Qualified ID> as <Valid ID> <Block>
        @BreakControl_BREAK = 178, // <BreakControl> ::= BREAK
        @ContinueControl_CONTINUE = 179, // <ContinueControl> ::= CONTINUE
        @Procedure = 180, // <Procedure> ::= <UnsetProc>
        @Procedure2 = 181, // <Procedure> ::= <PrintProc>
        @Procedure3 = 182, // <Procedure> ::= <ReturnProc>
        @Procedure4 = 183, // <Procedure> ::= <AssertProc>
        @Procedure5 = 184, // <Procedure> ::= <FailProc>
        @Procedure6 = 185, // <Procedure> ::= <ListVars>
        @Procedure7 = 186, // <Procedure> ::= <Call Generic>
        @UnsetProc_UNSET = 187, // <UnsetProc> ::= UNSET <Qualified ID>
        @PrintProc_PRINT = 188, // <PrintProc> ::= PRINT <Expression>
        @ReturnProc_RETURN = 189, // <ReturnProc> ::= RETURN <Expression>
        @ReturnProc_RETURN2 = 190, // <ReturnProc> ::= RETURN
        @AssertProc_ASSERT = 191, // <AssertProc> ::= ASSERT <Expression>
        @FailProc_FAIL = 192, // <FailProc> ::= FAIL <Expression>
        @FailProc_FAIL2 = 193, // <FailProc> ::= FAIL
        @ListVars_VARS = 194, // <ListVars> ::= VARS
        @CallGeneric_Identifier = 195 // <Call Generic> ::= Identifier <Argument Array>
    }
}