...
namespace Prometheus.Grammar
{
	...
	public enum GrammarRule
	{
		...
		@FlowControl7 = 164,                             // <FlowControl> ::= <EachControl>
		...
		@ForControl_for_LParen_RParen = 173,             // <ForControl> ::= for '(' <ForDeclare> <ForExpression> <ForStep> ')' <Block>
		@ForDeclare = 174,                               // <ForDeclare> ::= <Variable Statements> <End>
		@ForDeclare2 = 175,                              // <ForDeclare> ::= <End>
		@ForExpression = 176,                            // <ForExpression> ::= <Expression> <End>
		@ForExpression2 = 177,                           // <ForExpression> ::= <End>
		@ForStep = 178,                                  // <ForStep> ::= <Variable Statements>
		@ForStep2 = 179,                                 // <ForStep> ::= 
		@EachControl_each = 180,                         // <EachControl> ::= each <Qualified ID> <Block>
		@EachControl_each_as = 181,                      // <EachControl> ::= each <Qualified ID> as <Valid ID> <Block>
		@BreakControl_BREAK = 182,                       // <BreakControl> ::= BREAK
		@ContinueControl_CONTINUE = 183,                 // <ContinueControl> ::= CONTINUE
		@Procedure = 184,                                // <Procedure> ::= <UnsetProc>
		@Procedure2 = 185,                               // <Procedure> ::= <PrintProc>
		@Procedure3 = 186,                               // <Procedure> ::= <ReturnProc>
		@Procedure4 = 187,                               // <Procedure> ::= <AssertProc>
		@Procedure5 = 188,                               // <Procedure> ::= <FailProc>
		@Procedure6 = 189,                               // <Procedure> ::= <ListVars>
		@Procedure7 = 190,                               // <Procedure> ::= <Call Generic>
		@UnsetProc_UNSET = 191,                          // <UnsetProc> ::= UNSET <Qualified ID>
		@PrintProc_PRINT = 192,                          // <PrintProc> ::= PRINT <Expression>
		@ReturnProc_RETURN = 193,                        // <ReturnProc> ::= RETURN <Expression>
		@ReturnProc_RETURN2 = 194,                       // <ReturnProc> ::= RETURN
		@AssertProc_ASSERT = 195,                        // <AssertProc> ::= ASSERT <Expression>
		@FailProc_FAIL = 196,                            // <FailProc> ::= FAIL <Expression>
		@FailProc_FAIL2 = 197,                           // <FailProc> ::= FAIL
		@ListVars_VARS = 198,                            // <ListVars> ::= VARS
		@CallGeneric_Identifier = 199                    // <Call Generic> ::= Identifier <Argument Array>
	}
}
